  ...
  for (const auto& filter_chain : config.filter_chains()) {
    ...
    if (!filter_chain.has_transport_socket()) {
      if (filter_chain.has_tls_context()) {
        transport_socket.set_name(Config::TransportSocketNames::get().TLS);
    ...
    if (!filter_chain.filter_chain_match().transport_protocol().empty() &&
        filter_chain.filter_chain_match().transport_protocol() != transport_socket.name()) {
      throw EnvoyException(fmt::format("error adding listener '{}': transport protocol '{}' "
                                       "doesn't match configured transport socket '{}'",
                                       address_->asString(),
