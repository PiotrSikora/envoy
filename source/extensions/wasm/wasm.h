...
public:
  ...
  void setTickPeriod(std::chrono::milliseconds tick_period) { tick_period_ = tick_period; }

  // For testing only.
  void tickHandler();
  void setContext(Context* context) { context_.reset(context); }

private:
  std::unique_ptr<Common::Wasm::WasmVm> wasm_vm_;
  std::unique_ptr<Context> context_;
  std::function<void(Common::Wasm::Context*, uint32_t configuration_ptr,
